...
PRO
PRO
--select 'expdp \''/ as sysdba\'' TRANSPORT_TABLESPACES='||s.tablespace_name||' DIRECTORY=dp_for_cloud DUMPFILE=TTS_'||tablespace_name||'.dmp'||' LOGFILE=TTS'||tablespace_name||'_exp.log' from dba_tablespaces s where 1=1
--   AND s.contents NOT IN ('UNDO','TEMPORARY')
--   AND s.tablespace_name NOT IN ('SYSTEM','SYSAUX','TEMP','TEMPORARY','RBS','ROLLBACK','ROLLBACKS','RBSEGS','USERS') order by tablespace_name
...
PRO
PRO
--select 'impdp \''/ as sysdba\'' DIRECTORY=dp_from_onprem DUMPFILE=TTS_'||tablespace_name||'.dmp'||' LOGFILE=TTS'||tablespace_name||'_imp.log TRANSPORT_DATAFILES='||'&&dpdump_for_onprem./'||substr(file_name,instr(file_name,'/',-1)+1,length(file_name)) from dba_data_files s where 1=1
--   AND s.tablespace_name NOT IN ('SYSTEM','SYSAUX','TEMP','TEMPORARY','RBS','ROLLBACK','ROLLBACKS','RBSEGS','USERS')
--   AND s.tablespace_name NOT IN (SELECT tablespace_name
--                                   FROM dba_tablespaces
...
PRO Set environment variables to the database
PRO
select 'impdp system@pdbXX DIRECTORY=dp_from_onprem DUMPFILE=TTS_'||'01'||'.dmp'||' LOGFILE=TTS'||'01'||'_imp.log TRANSPORT_DATAFILES='||dataf
from(
