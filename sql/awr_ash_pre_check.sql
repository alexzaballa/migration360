...
--              Workload Repository (AWR) before executing mig360
--
...
--
-- Based on Carlos Sierra's script            
---------------------------------------------------------------------------------------
--
SET TERM ON;
SET HEA ON; 
...
DEF ash_mig360_days = '31';
DEF ash_fts_on_mig360 = '25';
...
-- range of snapshots for mig360
SELECT NVL(TO_CHAR(MIN(snap_id)), '0') ash_min_snap_id
  FROM sys.wrm$_snapshot 
 WHERE dbid = &&this_dbid. 
   AND begin_interval_time > SYSTIMESTAMP - &&ash_mig360_days.
/
...
COL mig360_fts_seconds NEW_V mig360_fts_seconds NOPRI;
SELECT TO_CHAR(CEIL((GREATEST(:ash_time_t1 - :ash_time_t0, 1)) / 100)) mig360_fts_seconds FROM DUAL
/

COL mig360_estimated_hrs NEW_V mig360_estimated_hrs NOPRI;
SELECT TO_CHAR(CEIL(&&ash_fts_on_mig360. * &&mig360_fts_seconds. * &&number_of_instances. * 2 / 3600)) mig360_estimated_hrs FROM DUAL
/
...
PRO If older than a week then mig360 may take long to execute 
PRO (suboptimal execution plans).
PRO
PRO There are &&ash_days_of_history. day(s) of history in ASH.
PRO If more than 31 days then mig360 may take long to execute 
...
PRO If older than a month then mig360 may take long to execute 
PRO (a median older than half the retention days usually means purging not happening).
PRO
PRO ASH size is &&ash_size_gb.
PRO If larger than 1 GB then mig360 may take long to execute 
PRO (mig360 may timeout after default threshold of 24hrs). 
PRO
PRO Last DDL on ASH objects is &&last_ddl_age_days. day(s) old (&&seg_last_ddl_time.).
PRO If older than a day then mig360 may take long to execute 
...
PRO If over 50% then mig360 may take long to execute 
...
PRO One scan of &&ash_mig360_days. day(s) of data takes &&mig360_fts_seconds. second(s) on this database.
PRO mig360 performs over &&ash_fts_on_mig360. scans of &&ash_mig360_days. day(s) of data on ASH per instance.
PRO About 1/2 of mig360 work load is access to ASH.
PRO
PRO *******************************************************************************
PRO *** Estimated ballpark execution time for mig360 is &&mig360_estimated_hrs. hour(s), 
PRO *** that is assuming optimal execution plans (with representative CBO stats).
PRO *******************************************************************************
PRO
PRO If pre-check suggests mig360 may take long to execute (over 24 hrs), 
PRO then take care first of ASH state:
PRO 1. If stats are outdated, use gather_stats_wr_sys.sql.
...
COL db_name CLE;
COL mig360_estimated_hrs CLE;
COL mig360_fts_seconds CLE;
COL ignore_this CLE;
